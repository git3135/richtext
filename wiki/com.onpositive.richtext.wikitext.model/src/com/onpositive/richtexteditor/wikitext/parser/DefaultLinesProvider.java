...
public class DefaultLinesProvider implements ILinesProvider {
	...
	public DefaultLinesProvider(String initialText,
			ILineMarkupParser lineMarkupParser, boolean canMergeLines) {
	...
	protected void createSpecialMarkupMatchers() {
		specialMarkupMatchers.add(new CitationMarkupMatcher(
				LineTextLexEvent.CITATION, '>'));
	}
			...
			if (next > pos) {
				...
				if (current.trim().length() == 0) // empty line
					...
					if (region != null) {
						...
						WikitextLine parsedLine = lineMarkupParser
								.parseLine(current);
						if (!wasBreak && isSameLineWithLast(parsedLine)) {
							appendToPreviousLine(parsedLine);
						} else
							addLine(parsedLine);
						if (parsedLine.getFullParsedLength() != current
								.length())
							addLineRest(current,
									parsedLine.getFullParsedLength());
					}
					wasBreak = false;
				}
			}
	...
	protected WikitextLine tryToParseRegion(int pos) {
		for (Iterator<IRegionParser> iterator = regionParsers.iterator(); iterator
				.hasNext();) {
	...
	public boolean isSameLineWithLast(WikitextLine line) {
		...
		if (line.getBullet() != null)
			return false;
		if (line.getClass() != WikitextLine.class
				|| previousLine.getClass() != WikitextLine.class)
			return false;
		if (line.getType() != WikitextLine.SIMPLE
				|| previousLine.getType() != WikitextLine.SIMPLE)
			return false;
	...
	protected boolean hasSpecialMarkup(WikitextLine line) {
		for (Iterator<ISyntaxMatcher> iterator = specialMarkupMatchers
	...
	protected void addLineRest(String current, int firstUnparserCharIdx) {
		try {
			if (current.length() <= firstUnparserCharIdx) {
				firstUnparserCharIdx = current.length();
			}
