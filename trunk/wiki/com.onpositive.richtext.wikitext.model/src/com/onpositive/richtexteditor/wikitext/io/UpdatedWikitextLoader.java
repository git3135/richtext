...
public class UpdatedWikitextLoader extends AbstractTextLoader
{
	...
	public UpdatedWikitextLoader(AbstractLayerManager manager)
	{
	...
	{
		DefaultLinesProvider provider = new DefaultLinesProvider(text, canMergeLines);
		final List<WikitextLine> linesList = provider.getLinesList();		
		final WikitextLexEventConsumer consumer = new WikitextLexEventConsumer(layer != null?layer.getDoc():null);
	...
	{
		return (lineBullets.size() == 0 || lineBullets.get(lineBullets.size() - 1) != currentList) && //First bullet for list   
			(bullet.getType() == OrderedListBullet.NUMBER && !bullet.getBulletText().toLowerCase().trim().equals("1.") || 
			bullet.getType() == OrderedListBullet.ROMAN && !bullet.getBulletText().toLowerCase().trim().equals("i.") ||
			bullet.getType() == OrderedListBullet.ABC && !bullet.getBulletText().toLowerCase().trim().equals("a."));
	}
	...
	/**
	 * @return the canMergeLines
	 */
	public boolean isCanMergeLines()
	{
		return canMergeLines;
	}

	/**
	 * @param canMergeLines the canMergeLines to set
	 */
	public void setCanMergeLines(boolean canMergeLines)
	{
