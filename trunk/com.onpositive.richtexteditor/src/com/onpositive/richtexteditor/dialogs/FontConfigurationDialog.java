...
public class FontConfigurationDialog extends org.eclipse.jface.dialogs.Dialog implements SelectionListener, ISelectionChangedListener, IPropertyChangeListener
{
	...
	public void selectionChanged(SelectionChangedEvent event)
	{
		if (event.getSource() == viewer)
		{
			...
			if (selectedStyle.getColor() != null)
			{
				...
				foregroundColorSelector.setColorValue(MetaConverter.convertRGB(selectedStyle.getColor()));
				colorManager.getColor(MetaConverter.convertRGB(selectedStyle.getColor()));
			}
			else
				foregroundColorSelector.setColorValue(new RGB(0,0,0));
			...
			{
				colorCheckbox.setSelection(true);
				backgroundColorSelector.setColorValue(MetaConverter.convertRGB(selectedStyle.getBgColor()));
				colorManager.getColor(MetaConverter.convertRGB(selectedStyle.getBgColor()));
			}
			else
				backgroundColorSelector.setColorValue(new RGB(255,255,255));
	...
	public void widgetSelected(SelectionEvent e)
	{
		...
		{			
			...
			dialog.setRGB(MetaConverter.convertRGB(getSelectedStyle().getColor()));
			dialog.open();
			FontData[] fontList = dialog.getFontList();
			String fontDataName = getSelectedStyle().getFontDataName();
			if (fontList != null &&
				fontRegistry.getFontData(fontDataName)!=fontList)
			{
				fontRegistry.put(fontDataName, dialog.getFontList());
			...
			if (dialog.getRGB() != null && !dialog.getRGB().equals(getSelectedStyle().getColor()))
			{
				...
				getSelectedStyle().setColor(MetaConverter.convertRGBBack(dialog.getRGB()));
				data.addChangedStyle(getSelectedStyle());
			}
			updatePreview();
		}
		...
		else if (e.getSource() == deleteButton)
		{
			...
			{
				ErrorDialog.openError(this.createShell(),"Cant delete this style", "Cant delete this style.", new Status(Status.INFO,"richtexteditor",IStatus.OK, "Default style cannot be deleted.", null));
				return;
			}
	...
	public void propertyChange(PropertyChangeEvent event)
	{
			...
			&& !foregroundColorSelector.getColorValue().equals(getSelectedStyle().getColor()))
		{
			...
			colorCheckbox.setSelection(true);
			getSelectedStyle().setColor(MetaConverter.convertRGBBack(color));
			data.addChangedStyle(getSelectedStyle());
			updatePreview();
		}
				...
				&& !backgroundColorSelector.getColorValue().equals(getSelectedStyle().getBgColor()))
		{
			...
			colorCheckbox.setSelection(true);
			getSelectedStyle().setBgColor(MetaConverter.convertRGBBack(color));
			data.addChangedStyle(getSelectedStyle());
			updatePreview();
		}		
	}
	...
	protected void updatePreview()
	{
		...
		if (selectedStyle.getColor() != null) fgColor = colorManager.getColor(MetaConverter.convertRGB(selectedStyle.getColor()));
		if (selectedStyle.getBgColor() != null) bgColor = colorManager.getColor(MetaConverter.convertRGB(selectedStyle.getBgColor()));
	...
	protected String getCorrectStyleName(String displayName)
	{
		...
		if (Character.isJavaIdentifierStart(displayName.charAt(0))) resStr.append(displayName.charAt(0));
			else resStr.append("_");
		for (int i = 1; i < displayName.length(); i++)
		{
			if (Character.isJavaIdentifierPart(displayName.charAt(i)))
				resStr.append(displayName.charAt(i));		
		}
		if (resStr.length() < 2) resStr.append("style");
		return checkForEqualStyleNames(resStr);
	}
	
	protected String checkForEqualStyleNames(StringBuilder resStr)
	{
		StringBuilder d=resStr;
		...
		do
		{
			...
			for (Iterator<FontStyle> iterator = data.getFontStyles().iterator(); iterator.hasNext();)
			{
				...
				if (name.equals(d.toString())) needToBeChanged = true;			
			}
			if (needToBeChanged) d.append('1');
		}
