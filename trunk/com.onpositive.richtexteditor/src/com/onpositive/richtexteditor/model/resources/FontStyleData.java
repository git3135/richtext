...
/**
 * @author kor Contains font style data duplicate to edit in customization
 *         dialog
 */
public class FontStyleData {
	 ...
	 * 
	 * @param initialData
	 *            Initial font style list to copy to this buffer
	 * @param parentFontRegistry
	 *            initial fort registry to copy to this buffer
	 */
	public FontStyleData(ArrayList<FontStyle> initialData,
			DisposableFontRegistry parentFontRegistry) {
	...
	/**
	 * @param resultFontRegistry
	 *            the resultFontRegistry to set
	 */
	 ...
	 * 
	 * @param manager
	 *            containing old list (currently used by document)
	 * @return Changed Styles list after validating
	 */
	public ArrayList<FontStyle> validateChangedStyles(IFontStyleManager manager) {
		for (int i = 0; i < changedStyles.size(); i++) {
			FontStyle fontStyle = (FontStyle) changedStyles.get(i);
			FontStyle style2 = ((FontStyleManager)manager).getStyleMap().get(
					fontStyle.getFontDataName());
			if (style2 != null) {
				FontData[] oldFD = parentFontRegistry.getFontData(style2
						.getFontDataName());
				FontData[] fontData = resultFontRegistry
						.getFontData(fontStyle.getFontDataName());
				if (style2.equals(fontStyle)
						&& oldFD[0].equals(fontData[0]))
					changedStyles.remove(i--);
			}
		}
